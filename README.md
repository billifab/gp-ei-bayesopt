...
Version 2.0 adds an operating-characteristic simulation framework BayesOpt-versus-RSM superiority validation, contributed by Yiqun Jiang (UCLA). Code and outputs live in the `aim3_power/` subfolder; see `aim3_power/README.md` for methodology and reproducibility notes. The original v1.0 pre-validation framework (this file) is retained unchanged.
